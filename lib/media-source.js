...
function createMediaSource (src, opt) {
  ...
  if (hasLoopStart || hasLoopEnd) {
    // start audio element at loopStart
    if (hasLoopStart) {
    ...
    window.requestAnimationFrame(function update () {
      ...
      if (hasLoopStart && loopStart > 0 && currentTime < loopStart) {
        // skip ahead to loop start
        audio.currentTime = loopStart
      } else if (hasLoopEnd) {
        // restart to loop start or zero
        var endTime = Math.min(audio.duration, loopEnd)
